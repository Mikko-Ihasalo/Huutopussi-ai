...
class Deck:
    ...
    def __init__(self):
        self.cards = {(suit,rank):Card(suit, rank) for suit in range(4) for rank in range(9)} # Add cards to deck with suit and rank
...
class PlayTick:
    def __init__(self, player:Player,hand:Hand):
        ...
        self.played_cards.clear_hand()
        self.triumph_card = -1


    def play_card(self,card:Card):
        played_card =self.player.hand.remove_card(card)
...
class Game:
    ...
    def play_tick(self, winner_n):  # Single tick to play
        ...
        for card in played_cards:
            self.players[winner_n].won_cards.add_card(card) # add cards to winner's hand
